...
### 3.5 Brand-Platform Union Logic (v5.1 Update)
**背景**: 傳統的篩選器採取 **AND (交集)** 邏輯，即 `Brand=Bosch` AND `Platform=Atika`。因大多數平台為單一品牌專屬，此邏輯會導致結果為空（無法同時滿足）。
**需求**: 用戶希望能簡單實現「顯示 A 品牌的平台，**加上** B 平台的產品」的疊加效果。

**解法**: 改採 **Union (聯集)** 邏輯架構，定義 Brand 與 Platform 的非對稱關係。

**核心規則**:
1.  **Brand as Controller** (連動控制):
    *   **角色**: Brand 篩選器不再直接參與資料過濾，而是作為 **Platform 篩選器的遙控器**。
    *   **行為**:
        *   勾選 `Brand X` -> 自動 **勾選** 所有屬於 Brand X 的 Platform。
        *   取消 `Brand X` -> 自動 **取消** 所有屬於 Brand X 的 Platform。

2.  **Platform as Filter** (核心過濾):
    *   **角色**: 唯一的過濾執行者。資料是否顯示，完全取決於 Platform 是否被勾選。
    *   **行為**: `filterSelections` 忽略 Brand Key，只讀取 Platform Key。
    *   **疊加能力**: 用戶可在由 Brand 自動勾選的基礎上，**手動加選** 其他 Platform。
    *   **範例**: 選 `Bosch` (自動勾選 Bosch 平台) + 手動勾選 `Atika`。結果顯示：Bosch 平台產品 + Atika 平台產品。

3.  **Visual Feedback** (勾選狀態):
    *   **問題**: 傳統邏輯下，勾選無資料的項目會變淡 (Opacity 0.5)，導致誤以為「被取消」。
    *   **修正**: 只要 Checkbox 是 **Checked** 狀態，無論當下是否有效，**一律顯示 Opacity 1.0 (全亮)**。只有「Unchecked 且 無效」的項目才顯示 Opacity 0.5。

---

## 11. 部署流程 (Deployment)

### 11.1 GitHub Push with Obfuscation
**目的**: 將原始碼發布至 GitHub，但避免原始碼被輕易閱讀或篡改。
**工具**: `deploy_encrypt.py` (Python Script)

### 11.2 工作流 (Workflow)
系統採用「原地加密後還原」的策略，確保 GitHub 上是加密版，而本地 OneDrive 始終保持可編輯的原始碼。

1.  **Backup**: 腳本啟動時，先將 `PT Landscape.html` 複製為 `.bak`，防止意外損壞。
2.  **Encrypt**:
    *   讀取 HTML 內容。
    *   將所有字元轉換為 Hex Encoding (`%XX` or `%uXXXX`)。
    *   生成包含 `document.write(unescape("..."))` 的 Wrapper HTML。
3.  **Overwrite**: 將加密後的內容寫回 `PT Landscape.html`。
4.  **Push**:
    *   執行 `git add PT Landscape.html`
    *   執行 `git commit -m "Deploy encrypted PT Landscape.html"`
    *   執行 `git push origin main`
5.  **Restore**:
    *   **關鍵步驟**: 立即將 `.bak` 內容寫回 `PT Landscape.html`。
    *   刪除備份檔。

### 11.3 操作指令
在 `PT_DATA工具` 或根目錄下開啟終端機：
```bash
python deploy_encrypt.py
```
若執行成功，將顯示 `Done! File restored to original state.`。
