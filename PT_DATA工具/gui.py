...
class App(ctk.CTk):
    ...
    def fetch_gs_sheets(self):
        ...
        def task():
            try:
                url = f"{APPS_SCRIPT_URL}?action=getSheets"
    ...
    def load_from_gs(self):
        ...
        def task():
            try:
                url = f"{APPS_SCRIPT_URL}?action=getData&sheet={requests.utils.quote(sheet_name)}"
    ...
    def upload_to_gs(self):
        ...
        # Get current sheet name or create new one
        sheet_name = self.gs_sheet_var.get()
        model = self.global_model_var.get()
        today = datetime.now().strftime("%Y%m%d")
        
        # If no valid sheet selected, create new sheet name
        if not sheet_name or sheet_name in ["(連線中...)", "(無分頁)"]:
            new_sheet_name = f"{model}_{today}"
        else:
            # Ask user: overwrite current sheet or create new?
            # For now, we'll create a new dated sheet
            new_sheet_name = f"{model}_{today}"
        
        self.log_message(f"正在上傳到 Google Sheets: {new_sheet_name}")
        ...
        def task():
            try:
                ...
                self.log_message("✓ 已連接到 Google Sheets API")
                
                # Open the spreadsheet
                spreadsheet = client.open_by_key(GOOGLE_SHEET_ID)
                self.log_message(f"✓ 已開啟試算表: {spreadsheet.title}")
                
                # Check if sheet exists, delete if so
                try:
                    existing_sheet = spreadsheet.worksheet(new_sheet_name)
                    spreadsheet.del_worksheet(existing_sheet)
                    self.log_message(f"✓ 已刪除舊分頁: {new_sheet_name}")
                except gspread.exceptions.WorksheetNotFound:
                    pass
                
                # Convert editor_data back to DataFrame
                df = pd.DataFrame(self.editor_data)

                # Define Column Groups based on user preference
                # Order: Priority (Metadata) -> Standard -> Dates (Desc) -> Others
                priority_cols = ["Repeat check", "Image", "Price Tag"]
                standard_cols = [
                ...
                # Identify date columns (8-digit YYYYMMDD) and sort DESCENDING (Newest First)
                date_cols = [c for c in all_cols if len(c) == 8 and c.isdigit() and c.startswith("20")]
                date_cols.sort(reverse=True)
                
                # Identify any other columns
                known_set = set(priority_cols + standard_cols + date_cols)
                extra_cols = [c for c in all_cols if c not in known_set]
                extra_cols.sort()
                
                # Construct Final Order
                final_order = []
                for c in priority_cols:
                    if c in df.columns: final_order.append(c)
                
                for c in standard_cols:
                    if c in df.columns: final_order.append(c)
                    
                final_order.extend(date_cols)
                ...
                self.log_message(f"✓ 已建立新分頁: {new_sheet_name} ({rows} 行 x {cols} 欄)")
                ...
                # Upload
                self.log_message(f"正在寫入 {len(all_data)} 筆資料...")
                new_worksheet.update('A1', all_data, value_input_option='RAW')
                
                self.log_message(f"✅ 成功上傳到分頁: {new_sheet_name}")
                
                # Refresh sheet list
                self.after(0, self.fetch_gs_sheets)
                    
            except FileNotFoundError:
                self.log_message(f"❌ 找不到 Service Account JSON: {SERVICE_ACCOUNT_FILE}")
            except gspread.exceptions.APIError as e:
                self.log_message(f"❌ Google API 錯誤: {e}")
            except Exception as e:
